...
#define MAX_STRING_LENGTH_VC 1000
...
long long min_count_vc = 1; // min occurrences for inclusion in vocab
...
void get_counts(char **rtrain_file,char **routput_file) {
    long long i = 0, j = 0, vocab_size = 12500;
    char format[20];
    char str[MAX_STRING_LENGTH_VC + 1];
...
void vocab_count(char **rfile_name,char **rvocab_file,int* rverbose,int* rmax_vocab, int* rmin_count) {
  ...
  /*  if (argc == 1) {
    ...
    */
    if (rverbose[0] != 2) verbose = rverbose[0];
    if (rmax_vocab[0] != 0) max_vocab = rmax_vocab[0];
    if (rmin_count[0] != 1 ) min_count_vc = rmin_count[0];
    get_counts(rfile_name,rvocab_file);
}
